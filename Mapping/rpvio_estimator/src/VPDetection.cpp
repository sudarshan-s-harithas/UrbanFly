...
void VPDetection::run( std::vector<KeyLine> &lines, cv::Point2d pp, double f, std::vector<cv::Point3d> &vps, std::vector<std::vector<int> > &clusters )
{
	this->lines = lines;
	this->pp = pp;
	this->f = f;
...
void VPDetection::getVPHypVia2Lines( std::vector<std::vector<cv::Point3d> > &vpHypo )
{
	...
	for ( int i=0; i<num; ++i )
	{
		cv::Mat_<double> p1 = ( cv::Mat_<double>(3, 1) << lines[i].startPointX, lines[i].startPointY, 1.0 );
		cv::Mat_<double> p2 = ( cv::Mat_<double>(3, 1) << lines[i].endPointX, lines[i].endPointY, 1.0 );

		lineInfos[i].para = p1.cross( p2 );

		double dx = lines[i].startPointX - lines[i].startPointY;
		double dy = lines[i].endPointX - lines[i].endPointY;
