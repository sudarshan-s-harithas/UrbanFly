...
flags.DEFINE_boolean("use_preprocessed", True,
                     'if use the propocessed data we provided for test')
flags.DEFINE_string("dataset_dir", '', "Filtered Dataset directory")
flags.DEFINE_string("output_dir", '', "Output directory")
flags.DEFINE_string("gpu", "0", "GPU ID")
flags.DEFINE_string("test_list", 'data_pre_processing/SYNTHIA/tst_100.txt',
                    "Test list")
flags.DEFINE_string("ckpt_file", 'pre_trained_model/synthia_498000',
                    "checkpoint file")
FLAGS = flags.FLAGS
...
focalLength_x = 320 * (320 / 640)
focalLength_y = 320 * (192 / 480)
centerX = 160.000000
centerY = 96.000000
...
def thres_mask(pred_masks, num_plane):
    ...
    max_mask = np.max(pred_masks,
                      axis=-1)  # here for depth prediction, only plane possiblity is used
    for p in range(num_plane + 1):
        ...
        thres_mask[:, :, p] = np.where(plane_mask == max_mask,
                                       thres_mask[:, :, p] + 1.,
                                       thres_mask[:, :, p])
...
def meshgrid(height, width, is_homogeneous=True):
    ...
    """
    x_t = np.matmul(np.ones(shape=[height, 1]),
                    np.expand_dims(np.linspace(-1., 1, width), 1).T)
...
def compute_depth(img, pred_param, num_plane, intrinsics):
    ...
    for k in range(num_plane):
        ...
        scale = 1. / (np.matmul(n_div_d, np.matmul(np.linalg.inv(intrinsics),
                                                   cam_coords)) + 1e-10)
...
def compute_errors(gt, pred):
    ...
    b_non_zero = np.all(
        gt * pred)  # if one of them are 0 this will return false
    if b_empyt or not b_non_zero:
        return [-100, -100, -100, -100, -100, -100,
                -100]  # this tst image will be ignored
...
def main(_):
    with open(TEST_LIST, 'r') as f:
        ...
        for t in test_files:
            ...
            if not FLAGS.use_preprocessed:
                ...
                if t_split[
                    0] == '22':  # seq 22 is not available in our preprocessed dataset, see README for more details
                    continue
                test_files_list.append(
                    FLAGS.dataset_dir + '/' + t_split[0] + '/' + t_split[-1])
                # depth_file_list.append(FLAGS.dataset_dir + '/' + t_split[0] +'/'+ t_split[-1] + '_depth.png')
    ...
    default_top_five_colors = [(0.8, 0.0, 1.0), (0.8, 1.0, 0.0),
    ...
    with tf.Session(config=config) as sess:
        ...
        for t in range(0, len(test_files_list), FLAGS.batch_size):
            if t % 100 == 0:
                ...
                basename, t, len(test_files_list)))
            inputs = np.zeros(
            ...
            avg_time += cost_time
            print("No.%d batch cost_time: %f/img" % (
            int(t / FLAGS.batch_size), cost_time / FLAGS.batch_size))
            ...
            for b in range(FLAGS.batch_size):
                ...
                color_plane_mask = color_mask(inputs[b],
                                              pred['pred_mask'][b, :, :, :],
                                              colors, alpha=0.6)
                ...
                for p in range(FLAGS.num_plane):
                    ...
                    combined_mask += (p + 1) * thres_masks[:, :,
                                               p: p + 1]  # for matlab to eval all the plane should start from 1, and the last channle will be 0 as non-plane
                ...
                cv2.imwrite(eval_mask_path + pic_name,
                            combined_mask)  # misc will normalize the number to 255 not good
                modified_combined_mask = combined_mask * 60
                cv2.imwrite(modified_eval_mask_path + pic_name,
                            modified_combined_mask)
                '''
