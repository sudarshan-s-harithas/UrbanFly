...
class PlaneRCNNDetector():
        ...
        if not separate:
            if options.startEpoch >= 0:
                self.model.load_state_dict(
                    torch.load(checkpoint_dir + '/checkpoint_' + str(options.startEpoch) + '.pth'))
            else:
        ...
        if 'refine' in modelType or 'final' in modelType:
            ...
            if not separate:
                ...
                self.refine_model.load_state_dict(
                    torch.load('checkpoint/instance_normal_refine_mask_softmax_valid/checkpoint_refine.pth'))
                pass
            pass
    ...
    def detect(self, sample):
        ...
        for indexOffset in [0, ]:
            ...
            input_pair.append(
                {'image': images, 'depth': gt_depth, 'mask': gt_masks, 'bbox': gt_boxes, 'extrinsics': extrinsics,
            ...
            detection_pair.append(
                {'XYZ': XYZ_pred, 'depth': XYZ_pred[1:2], 'mask': detection_mask, 'detection': detections,
        ...
        if ('refine' in self.modelType or 'refine' in self.options.suffix):
            ...
            pose_gt = torch.cat([pose[0:1], -pose[2:3], pose[1:2], pose[3:4], -pose[5:6], pose[4:5]], dim=0).unsqueeze(
                0)
            ...
            for c in range(1):
                ...
                detection_masks = torch.nn.functional.interpolate(detection_masks[:, 80:560].unsqueeze(1),
                                                                  size=(192, 256), mode='nearest').squeeze(1)
                ...
                segmentation = torch.nn.functional.interpolate(segmentation[:, 80:560].unsqueeze(1).float(),
                                                               size=(192, 256), mode='nearest').squeeze().long()
                ...
                if self.options.modelType == 'fitting':
                    ...
                    XYZ = torch.nn.functional.interpolate(ranges.unsqueeze(1), size=(192, 256),
                                                          mode='bilinear').squeeze(1) * results[-1]['depth'].squeeze(1)
                    ...
                    plane_parameters = torch.stack([torch.matmul(torch.inverse(AA[planeIndex]), Ab[planeIndex]) if
                                                    detection_areas[planeIndex] else detection_dict['detection'][
                                                                                     planeIndex, 6:9] for planeIndex in
                                                    range(len(AA))], dim=0)
                    ...
                    XYZ_pred, detection_mask, plane_XYZ = calcXYZModule(self.config, camera,
                                                                        detection_dict['detection'], detection_masks,
                                                                        detection_dict['depth'], return_individual=True)
...
class DepthDetector():
    ...
    def detect(self, sample):
        ...
        for indexOffset in [0, ]:
            ...
            depth_np_pred = self.model.predict([images, camera], mode='inference_detection', use_nms=2,
                                               use_refinement='refinement' in self.options.suffix)

            if depth_np_pred.shape != gt_depth.shape:
                depth_np_pred = torch.nn.functional.interpolate(depth_np_pred.unsqueeze(1), size=(640, 640),
                                                                mode='bilinear').squeeze(1)
                pass
...
class PlaneNetDetector():
    ...
    def detect(self, sample):
        ...
        for indexOffset in [0, ]:
            ...
            depth = np.concatenate([np.zeros((80, 640), dtype=np.int32), depth, np.zeros((80, 640), dtype=np.int32)],
                                   axis=0)
            ...
            detection_pair.append(
                {'depth': depth, 'mask': masks.sum(0, keepdim=True), 'masks': masks, 'detection': detections})
            continue
        return detection_pair
...
class PlaneRecoverDetector():
    ...
    def detect(self, sample):
        ...
        for indexOffset in [0, ]:
            ...
            detections = torch.from_numpy(detections).float().cuda()
            masks = torch.from_numpy(masks).float().cuda()
            XYZ_pred, detection_mask, plane_XYZ = calcXYZModule(self.config, camera, detections, masks,
...
class TraditionalDetector():
    ...
    def detect(self, sample):
        detection_pair = []
        for indexOffset in [0, ]:
            images, image_metas, rpn_match, rpn_bbox, gt_class_ids, gt_boxes, gt_masks, gt_parameters, gt_depth, extrinsics, planes, gt_segmentation, gt_semantics = \
            sample[indexOffset + 0].cuda(), sample[indexOffset + 1].numpy(), sample[indexOffset + 2].cuda(), sample[
                indexOffset + 3].cuda(), sample[indexOffset + 4].cuda(), sample[indexOffset + 5].cuda(), sample[
                indexOffset + 6].cuda(), sample[indexOffset + 7].cuda(), sample[indexOffset + 8].cuda(), sample[
            ...
            input_dict['info'] = np.array(
                [camera[0], 0, camera[2], 0, 0, camera[1], camera[3], 0, 0, 0, 1, 0, 0, 0, 0, 1, camera[4], camera[5],
                 1000, 0])
...
def evaluate(options):
    ...
    if options.dataset == '':
        ...
        camera[4] = 1242
        camera[5] = 375
        dataset = InferenceDataset(options, config, image_list=glob.glob('../../Data/KITTI/scene_3/*.png'),
                                   camera=camera)
        ...
        camera[4] = 640
        camera[5] = 480
        dataset = InferenceDataset(options, config, image_list=glob.glob('../../Data/SevenScene/scene_3/*.png'),
                                   camera=camera)
        ...
        camera[4] = 1
        camera[5] = 1
        dataset = InferenceDataset(options, config, image_list=glob.glob('../../Data/TankAndTemple/scene_4/*.jpg'),
                                   camera=camera)
        ...
        image_list = [filename for filename in image_list if
                      '63_image' not in filename and '77_image' not in filename] + [filename for filename in image_list
                                                                                    if
                                                                                    '63_image' in filename or '77_image' in filename]
        camera = np.zeros(6)
        camera[0] = 587
        camera[1] = 587
        camera[2] = 320
        ...
        image_list = sorted(
            glob.glob(options.customDataFolder + '/*.png') + glob.glob(options.customDataFolder + '/*.jpg'),
            key=alphanum_key)
        print(image_list)
    ...
    for name, detector in detectors:
        ...
        for sampleIndex, sample in enumerate(data_iterator):
            ...
            for indexOffset in [0, ]:
                ...
                input_pair.append({'image': images, 'depth': gt_depth, 'bbox': gt_boxes, 'extrinsics': extrinsics,
            ...
            if options.dataset == 'rob':
                ...
                cv2.imwrite(image_list[sampleIndex].replace('color', 'depth').replace('jpg', 'png'),
                            depth_rounded.astype(np.uint16))
                continue
            ...
            if sampleIndex < 30 or options.debug or options.dataset != '':
                visualizeBatchPair(options, config, input_pair, detection_pair, indexOffset=sampleIndex % 500,
                                   suffix='_' + name + options.modelType, write_ply=options.testingIndex >= 0,
    ...
    if 'inference' not in options.dataset:
        if options.debug and len(detectors) > 1:
            all_statistics = np.concatenate(
                [np.arange(len(all_statistics[0][0])).reshape((-1, 1)), ] + [np.array(statistics[3]) for statistics in
                                                                             all_statistics], axis=-1)
