...
class Planner:
    ...
    def __init__(self, vertices_msg, odometry_msg, local_goal_pub, local_stomp_pub, feasible_path_pub, feasible_path_pub2, free_cloud_pub, colliding_cloud_pub):
        ...
        self.feasible_path_pub2 = feasible_path_pub2
    ...
    def publish_local_stomp_paths(self):
        ...
        if optimal_line_strip is not None:
            ...
            if (not is_init_path) or (optimized_traj is not None):
                ...
                nav_path = Path()
                nav_path.header = self.odometry_msg.header

                for point in optimal_line_strip.points:
                    way_pt = PoseStamped()
                    way_pt.header = self.odometry_msg.header
                    way_pt.pose.position.x = point.x
                    way_pt.pose.position.y = point.y
                    way_pt.pose.position.z = point.z
